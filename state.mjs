...
if (c.archive_note && !/present/.test(c.archive_note)) console.log('ARCHIVE    ' + c.archive_note);
...
if (c.archive_note && !/present/.test(c.archive_note)) console.log('EMPTY      UNKNOWN — archive bar counts unavailable until the ALTER TABLE runs; ' + empty.length + ' symbols have no D1 bars');
else console.log('EMPTY      ' + empty.length + ' registered with no bars anywhere: ' + (empty.join(', ') || 'none'));
...
const logEntries = S.log && S.log.entries ? (Array.isArray(S.log.entries) ? S.log.entries : (S.log.entries.entries || [])) : [];
const warn = logEntries.filter(e => e.level !== 'info').slice(-5).map(e => e.t.slice(11, 16) + ' ' + e.code + ': ' + e.message).join('\n           ');
