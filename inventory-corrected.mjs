...
const SYMS = JSON.parse(readFileSync('qa-corrected/fetch-log.json','utf8')).filter(l=>l.result==='OK').map(l=>l.symbol);
const DATES = null;
...
writeFileSync('qa-corrected/master.csv', master.join('\n'));
const cols = Object.keys(inv[0]);
writeFileSync('qa-corrected-data-inventory.csv', [cols.join(',')].concat(inv.map(r => cols.map(c => JSON.stringify(r[c] == null ? '' : r[c])).join(','))).join('\n'));
writeFileSync('qa-corrected/duplicates.json', JSON.stringify(dupRows, null, 1));
const n = k => inv.filter(x => x.status === k).length;
console.log(`symbol-days: ${inv.length} found · COMPLETE ${n('COMPLETE')} · INCOMPLETE ${n('INCOMPLETE')} · UNAVAILABLE ${n('UNAVAILABLE')}`);
