...
// ---- /audit must catch what the unit tests cannot
{
  ...
  let r = await gA('/audit?force=1&date=' + D + '&symbols=CLEAN');
  ...
  r = await gA('/audit?force=1&date=' + D + '&symbols=BADOHLC');
  ...
  db.db.prepare("INSERT OR REPLACE INTO bars (symbol, unix, date, time, open, high, low, close, volume, first_seen, updated_at, revisions) VALUES ('WRONGDAY',?,?,'09:30',100,100.5,99.5,100,1000,0,0,0)")
    .run(base + 5 * 86400, D);
  r = await gA('/audit?force=1&date=' + D + '&symbols=WRONGDAY');
  ...
  for (let i = 92; i < 115; i++) put('LATE', String(9 + Math.floor((30 + i) / 60)).padStart(2, '0') + ':' + String((30 + i) % 60).padStart(2, '0'),
    base + i * 60, 100, 100.5, 99.5, 100, 1000);
  r = await gA('/audit?force=1&date=' + D + '&symbols=LATE');
  ...
  r = await gA('/audit?force=1&date=' + D + '&symbols=DUPE');
...
// ---- the audit must not become the heaviest thing running
{
  const eC = { DB: db, RATE_PER_MIN: 1000000, LOG: { get: async () => [], put: async () => {} } };
  const gC = async (p) => JSON.parse(await (await mod.fetch(new Request('https://x' + p), eC, ctx)).text());

  const first = await gC('/audit?force=1');
  const second = await gC('/audit');
  check('a second audit inside the interval is served from the stored verdict',
    typeof second.cached_seconds === 'number', JSON.stringify(second.cached_seconds));
  check('the cached verdict matches the computed one', second.verdict === first.verdict);
  check('force=1 recomputes', (await gC('/audit?force=1')).cached_seconds === undefined);

  const src = readFileSync(new URL('./worker.js', import.meta.url), 'utf8');
  check('the previous close is found through the days table, not a table scan',
    /SELECT date FROM days WHERE symbol = \? AND date < \?/.test(src));
  check('the unindexed scan is gone',
    !/SELECT close FROM bars WHERE symbol = \? AND date < \?/.test(src));
