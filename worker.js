...
import { VIEW_HTML, RADAR_HTML, DB_HTML, DATA_HTML, SCAN_HTML, BARS_HTML, REPLAY_HTML, TRADER_V2_HTML, TRADER_V2_QA_HTML, BUILD } from './view.js';
...
alter table archive_symbols add column if not exists bars bigint not null default 0;
alter table archive_symbols add column if not exists first_unix bigint;
alter table archive_symbols add column if not exists last_unix bigint;
...
// Per symbol: 1 fetch + up to 2 id lookups + 2 chunk writes + 2 summary
// (count, patch) = 7. Five symbols is 35, plus 4 to publish = 39 of 50.
// 24 nightly runs cover 120 symbols; a universe grown past that takes two
// nights, and 'fill the archive now' covers any size on demand.
const NIGHTLY_SHARD = 5;
...
async function archiveId(env, sym) {
  ...
  // 390 for CRDO that had never been fetched. Insert WITHOUT an id and read
  // back what the sequence assigned; on a conflict (the other isolate won)
  // read the row the other isolate made.
  const ins = await sb(env, 'archive_symbols?on_conflict=symbol', { method: 'POST',
    headers: { Prefer: 'resolution=ignore-duplicates,return=representation' },
    body: JSON.stringify([{ symbol: sym }]) });
  let row = null;
  try { const arr = JSON.parse(ins.text); row = Array.isArray(arr) ? arr[0] : null; } catch (e) { row = null; }
  if (!row || row.id == null) {
    const again = JSON.parse((await sb(env, 'archive_symbols?select=id,symbol&symbol=eq.' + sym)).text);
    row = again[0] || null;
  }
  if (!row || row.id == null) throw new Error('archive_symbols: could not obtain an id for ' + sym);
  archiveIds[sym] = row.id;
  return row.id;
}
...
async function archiveWrite(env, sym, bars) {
  ...
  // keep the per-symbol summary current, so coverage never has to count bars
  try {
    const cnt = await sb(env, 'archive_bars?select=unix&symbol_id=eq.' + id + '&limit=1', { headers: { Prefer: 'count=exact' } });
    const cr = cnt.headers.get('content-range') || ''; const n = cr.indexOf('/') >= 0 ? +cr.split('/')[1] : null;
    const lo = Math.min.apply(null, bars.map(b => b.unix)), hi = Math.max.apply(null, bars.map(b => b.unix));
    if (n != null) await sb(env, 'archive_symbols?id=eq.' + id, { method: 'PATCH',
      headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ bars: n, last_unix: hi, first_unix: lo }) });
  } catch (e) { /* the summary is a convenience; the bars are already written */ }
  return { written };
}
...
async function handle(req, env, ctx) {
  {
    ...
    // The QA team's report, served read-only alongside the lab it describes.
    if (route === 'trader-v2-qa' || (route === 'trader-v2' && a === 'qa')) {
      return new Response(TRADER_V2_QA_HTML, { headers: { ...H, 'Content-Type': 'text/html; charset=utf-8' } });
    }
    ...
    // THE ONE HONEST TABLE. For every symbol anyone has ever registered: how
    // many sessions each store actually holds. Not names — bars. Each number
    // is a count of rows, so a symbol registered with nothing shows 0/0/0
    // instead of appearing in a list that implies it has data.
    if (route === 'coverage') {
      ...
      const d1By = {}; d1.forEach(r => { d1By[r.symbol] = r; });
      const tracked = new Set((await trackedSymbols(db, env)).map(s => s.symbol));
      const UNI = await universeList(db);
      const rows = {};
      const touch = s => rows[s] = rows[s] || { symbol: s, live_tracked: tracked.has(s), in_universe: UNI.indexOf(s) >= 0,
        d1_days: 0, d1_bars: 0, archive_days: 0, archive_bars: 0, first: null, last: null };
      Object.keys(d1By).forEach(s => { const r = touch(s); r.d1_days = r.days = d1By[s].days; r.d1_bars = d1By[s].bars; r.first = d1By[s].first; r.last = d1By[s].last; });
      tracked.forEach(touch); UNI.forEach(touch);
      let archiveNote = 'archive not configured';
      if (mirrorOn(env)) {
        try {
          // one grouped read via an RPC-free path: page the id column of the
          // bars table is too big, so the archive keeps a per-symbol summary
          // in archive_symbols (bars, first_unix, last_unix) maintained on
          // write; if this deployment's table lacks those columns, fall back
          // to counting per symbol, which costs a request each.
          const syms = JSON.parse((await sb(env, 'archive_symbols?select=id,symbol,bars,first_unix,last_unix')).text);
          const hasSummary = syms.length && 'bars' in syms[0];
          for (const x of syms) {
            const r = touch(x.symbol);
            if (hasSummary) { r.archive_bars = x.bars || 0; r.archive_days = x.bars ? Math.round(x.bars / 390 * 10) / 10 : 0;
              if (x.first_unix) r.first = r.first || new Date(x.first_unix * 1000).toISOString().slice(0, 10);
              if (x.last_unix) r.last = new Date(x.last_unix * 1000).toISOString().slice(0, 10); }
          }
      ...
      const list = Object.values(rows).sort((a, b2) => a.symbol < b2.symbol ? -1 : 1);
      return json({ symbols: list, count: list.length,
        with_live_bars: list.filter(r => r.d1_bars > 0).length,
        with_archive_bars: list.filter(r => r.archive_bars > 0).length,
        registered_but_empty: list.filter(r => !r.d1_bars && !r.archive_bars).map(r => r.symbol),
        archive: archiveNote,
        note: 'bars are counted, not names. live = D1, 60 sessions; archive = Supabase archive_bars, 42 sessions; the mirror is a copy of D1 and is not shown.' });
    }
