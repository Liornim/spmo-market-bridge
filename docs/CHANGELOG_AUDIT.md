...
## v252 — UI action lineage (documentation + proof, no behaviour change)
- `docs/UI_ACTION_DATA_LINEAGE.md` / `.csv`: 85 actions across 11 served pages,
  31 backend endpoint records; generated by `docs/audit/lineage_build.mjs`.
- Coverage proof in `npm test`: all 367 inventory elements are mapped to an
  action or listed as no-data; the build fails on an unmapped element, a mapped
  line that no longer exists, an unknown endpoint key or an unresolved anchor.
  Both failure paths were negative-tested.
- Findings recorded in the records (not fixed here): `/export` vs `/bars/export`
  return different "all history"; CSV import shifts columns for `/export` files
  (LOCALLY VERIFIED); both radars share localStorage `bars-cache-v1`; production
  radar never picks up a revised minute; pack "Data source" line is hard-coded;
  `/trader-v2/live` drops the newest row after the close too; dead
  "הרץ Golden QA" button; `/view` and `/trader-v2/live` poll in background tabs.
