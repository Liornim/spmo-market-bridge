...
export const EP = {
  ...
  // ---- V2 pipeline endpoints (v274). All read-only; none touches a legacy table.
  v2_symbols: {
    route: 'GET /v2/symbols', anchor: "if (p0[0] === 'v2')",
    source: 'D1_V2', table: 'symbols_v2',
    sql: 'SELECT * FROM symbols_v2 ORDER BY symbol',
    output: '{symbols[{symbol,tier,active,last_fetch_at,last_bar_unix,last_error,last_ok_at}]}', side: 'none',
    d1: 'symbols_v2 (one row per tracked symbol)', kv: '0', provider: '0', mixed: 'D1_V2', tz: 'n/a', live: 'none',
  },
  v2_days: {
    route: 'GET /v2/days/:sym', anchor: "if (p0[0] === 'v2')",
    source: 'D1_V2', table: 'bars_v2',
    ...
    output: '{symbol,days,complete_days,incomplete_days,total_bars,rows[{date,bars,expected,missing,duplicates,synthetic,revisions,complete}]}', side: 'none',
    d1: 'bars_v2 rows of the symbol, aggregated', kv: '0', provider: '0', mixed: 'D1_V2', tz: 'ET date', live: 'none',
  },
  v2_day: {
    route: 'GET /v2/day/:sym/:date', anchor: "if (p0[0] === 'v2')",
    source: 'D1_V2', table: 'bars_v2',
    sql: 'SELECT * FROM bars_v2 WHERE symbol = ? AND date = ? ORDER BY unix',
    output: '{symbol,date,bars,rows[...canonical V2 candle incl. source, synthetic, first_seen, updated_at, revisions]}', side: 'none',
    d1: 'up to 390 rows', kv: '0', provider: '0', mixed: 'D1_V2', tz: 'ET', live: 'none',
  },
  v2_status: {
    route: 'GET /v2/status', anchor: "if (p0[0] === 'v2')",
    source: 'D1_V2', table: 'symbols_v2 + bars_v2 + jobs_v2 + runs_v2',
    sql: 'three aggregates: symbols, today\'s bars grouped by symbol, jobs ordered by due_at; plus the last 10 runs',
    output: '{version,market_open,system{tracked,request_budget,jobs_ready,jobs_failed,stale_symbols},symbols[...],recent_runs[...]}', side: 'none',
    d1: '4 statements regardless of universe size (no N+1)', kv: '0', provider: '0', mixed: 'D1_V2', tz: 'ET', live: 'none',
  },
  v2_compare: {
    route: 'GET /v2/compare', anchor: "if (p0[0] === 'v2')",
    source: 'MIXED', table: 'bars_v2 + bars (legacy, READ ONLY)',
    sql: 'two grouped aggregates over the same ET date, one per store',
    merge: 'joined per symbol in the Worker; neither store is modified', filter: 'legacy side restricted to canonical minutes',
    output: '{date,market_open,expected_latest_closed_minute,v2_bar_count_spread,truncation_check,rows[{symbol,v2_latest,legacy_latest,difference_minutes,v2_bars,legacy_bars,v2_synthetic}]}', side: 'none',
    d1: '2 statements', kv: '0', provider: '0', mixed: 'MIXED', tz: 'ET date', live: 'none',
  },
  v2_export: {
    route: 'GET /v2/export/:sym | /v2/export?symbols=|all=1', anchor: "if (p0[0] === 'v2')",
    source: 'D1_V2', table: 'bars_v2',
    sql: 'SELECT * FROM bars_v2 WHERE symbol = ? [AND date >= ?] [AND date <= ?] ORDER BY date, unix',
    output: 'CSV: symbol,date,time,unix,open,high,low,close,volume,source,synthetic,revisions,first_seen,updated_at', side: 'none',
    d1: 'every matching row of each requested symbol', kv: '0', provider: '0', mixed: 'D1_V2', tz: 'ET', live: 'none',
  },
  days: {
